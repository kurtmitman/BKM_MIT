...
%     YT      = ZT.*phi_ext(CT,params).*KT.^alpha.*exLT.^(1-alpha); 
    ...
    if params.budget_balance ==1
        ...
        BGT = BG_ss*ones(TT,1);
        for t=2:TT-1
            transT(t) = BGT(t+1)-(1+rtildeT(t))*BGT(t)-GT(t)+TaxT(t)+taxcT(t)*CT(t);
        end        
        t=TT;
        transT(TT) = BG_ss-(1+rtildeT(t))*BGT(t)-GT(t)+TaxT(t)+taxcT(t)*CT(t);
    else
        BGT = BG_ss*ones(TT,1);
        taxcT = ones(TT,1)*params.taxc;
        for t=2:TT-1
            %taxcT(t) = params.taxc + tax_val*(log(YT(t))-log(params.Yss)) + debt_val*(BGT(t)-BG_ss);
            BGT(t+1)=(1+rtildeT(t))*BGT(t)-taxcT(t)*CT(t)-TaxT(t);
        end        
    end
%     keyboard
